...
def main():
    ...
    history = final_round_acc(0, round_acc, history, logger, args, "Ours")
    ...
    for rnd in range(1, args.global_rounds + 1):
        ...
        client_uploads, round_acc = client_local_training(
            train_clients, id_to_dataset, round_acc, rnd, logger
        )
        history = final_round_acc(rnd, round_acc, history, logger, args, mode="Ours")

        server_update(server, client_uploads, logger)

        distribute_to_clients(server, train_clients)
 
        if not args.no_save_model and rnd % args.save_model_epoch == 0 :
            save_gen_model(server, args, rnd, logger)
