            ...
            for label_idx, label_name in enumerate(src_names):
                ...
                with torch.no_grad():
                    ...
                    if len(valid_probs_tgt) == 0:
                        logger.log(f"{src_id}:'{label_name}' -> {tgt_id} | ⚠️ [Filtered: All Target Clients High Uncertainty]")
                        continue
                    ...
                    if entropy_tgt > entropy_threshold:
                        ...
                        logger.log(f"{src_id}:'{label_name}' -> {tgt_id} | predict:'{pred_tgt_name}' | Entropy:{entropy_tgt:.2f} (Valid Clients: {len(valid_probs_tgt)}/{len(tgt_clients)})")

                    img_tgt_real = get_real_images(tgt_loader, pred_tgt_idx, num_samples=128)
                    ...
                    for c_model in src_clients:
                        ...
                        if ent <= entropy_threshold:
                            valid_probs_src.append(probs)

                    if len(valid_probs_src) == 0:
                        logger.log(f"  -> Check -> {src_id} | ❓ | ⚠️ [Filtered: All Source Clients Weak Cycle-Back]")
                        continue

                    avg_probs_src = torch.stack(valid_probs_src).mean(dim=0)
                    ...
                    if entropy_src > entropy_threshold:
                        ...
                        if match_symbol == "✅":
                            logger.log(f"[Real Img Match] {src_id}:'{label_name}' <==> {tgt_id}:'{pred_tgt_name}' ✅ (Valid Clients: {len(valid_probs_src)}/{len(src_clients)})")
...
def img_discover_label_mappings(local_id_to_global_id, generators_dict, clients_dict, label_space_meta, device, logger):
    ...
    for i, src_id in enumerate(dataset_ids):
        for j, tgt_id in enumerate(dataset_ids):
            ...
            max_ent_src = math.log(len(src_names))
            dynamic_thresh_src = max_ent_src * tolerance_ratio
            ...
            for label_idx, label_name in enumerate(src_names):
                ...
                with torch.no_grad():
                    ...
                    for c_model in tgt_clients:
                        ...
                        # ⭐ 篩選機制：只聽取有自信的考官
                        # 💡 [未來優化] 未來若要用動態門檻，請把這裡的 entropy_threshold 改成 dynamic_thresh_tgt
                        # if ent <= entropy_threshold:
                        #     valid_probs_tgt.append(probs)
                    ...
                    # 💡 [未來優化] 未來請改成 dynamic_thresh_tgt
                    # if entropy_tgt > entropy_threshold: 
                    #     logger.log(f"{src_id}:'{label_name}' -> {tgt_id} | predict:'{pred_tgt_name}' | Entropy:{entropy_tgt:.2f} ⚠️ [Filtered: Ensemble High Uncertainty]")
                    #     continue
                    # else: 
                    #     logger.log(f"{src_id}:'{label_name}' -> {tgt_id} | predict:'{pred_tgt_name}' | Entropy:{entropy_tgt:.2f} (Valid Clients: {len(valid_probs_tgt)}/{len(tgt_clients)})")

                    if entropy_tgt > dynamic_thresh_tgt:  
                        logger.log(f"{src_id}:'{label_name}' -> {tgt_id} | predict:'{pred_tgt_name}' | Entropy:{entropy_tgt:.2f} ⚠️ [Filtered: Ensemble High Uncertainty]")
                        continue
                    else: 
                        logger.log(f"{src_id}:'{label_name}' -> {tgt_id} | predict:'{pred_tgt_name}' | Entropy:{entropy_tgt:.2f} (Valid Clients: {len(valid_probs_tgt)}/{len(tgt_clients)})")
                    ...
                    for c_model in src_clients:
                        ...
                        # ⭐ 篩選機制：反向驗證同樣只聽取有自信的考官
                        # 💡 [未來優化] 未來請改成 dynamic_thresh_src
                        # if ent <= entropy_threshold:
                        #     valid_probs_src.append(probs)

                        if ent <= dynamic_thresh_src:  
                            valid_probs_src.append(probs)
                    ...
                    #     match_symbol = "✅" if pred_src_cycle_idx == label_idx else "❌"
                    #     if match_symbol == "✅":
                    #         logger.log(f"[Match] {src_id}:'{label_name}' <==> {tgt_id}:'{pred_tgt_name}' ✅ (Valid Clients: {len(valid_probs_src)}/{len(src_clients)})")
                    #         register_mapping(local_id_to_global_id, src_id, label_idx, tgt_id, pred_tgt_idx)

                    if entropy_src > dynamic_thresh_src: 
                        match_symbol = "❓"
                        logger.log(f"  -> Check -> {src_id} | Predict:'{pred_src_cycle_name}' | {match_symbol} | Entropy:{entropy_src:.2f} ⚠️ [Filtered: Weak Cycle-Back]")
                    else:
                        match_symbol = "✅" if pred_src_cycle_idx == label_idx else "❌"
