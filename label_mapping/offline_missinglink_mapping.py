...
def save_mapping_summary(path, mapping, label_space_meta, threshold):
    with open(path, "a", encoding="utf-8") as f:
        f.write("\n" + "#" * 50 + "\n")
        f.write(f"Missing Link Threshold = {threshold}\n")
        f.write("#" * 50 + "\n")
...
def run_offline(args):
    ...
    logger = SimpleLogger(args.output_dir)

    csv_path = os.path.join(args.output_dir, "offline_missing_link_mapping_acc.csv")
    ...
    for rnd in rounds:
        print("\n" + "=" * 60)
        print(f"Offline Missing Link | Global Round {rnd}")
        print("=" * 60)
        ...
        mapping_log_path = os.path.join(args.output_dir, f"round_{rnd}_mapping_summary.log")

        for threshold in thresholds:
            print(f"\n[Run] Round {rnd} | Missing Link threshold = {threshold}")
            ...
            print(
                f"Round {rnd} | threshold {threshold:.2f} | "
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default="./offline_missing_link_results")
    parser.add_argument("--seed", type=int, default=None, help="random seed")

    parser.add_argument("--rounds", type=str, default="5,10,15,20,25")
    parser.add_argument("--thresholds", type=str, default="0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.55,0.6,0.65,0.7,0.75,0.8,0.85,0.9,0.95,1.0")
