...
class Server(Base_Server):
    def __init__(self, **kwargs):
        ...
        # ConditionalGenerator (FedTED feature generator)
        ...
        # DDPM aggregated weights per dataset
        self.global_ddpm_states = {}

    def aggregate(self):
        #super().aggregate()

        # aggregate DDPM weights grouped by dataset
        ...
        self.logger.log(f"[Server] Aggregating DDPMs from {len(self.selected_clients)} clients "
    ...
    def distribute_model(self):
        ...
        for client in self.selected_clients:
            ...
            # distribute ConditionalGenerator and prox_z (FedTED)
            if self.global_feat_gen is not None:
            ...
            # distribute aggregated DDPM weights (DDPM)
            d_name = client.dataset_name
    ...
    def train_generator(self):
        """Train ConditionalGenerator in feature space (FedTED)."""
        all_global_ids = set()
        ...
        if self.global_feat_gen is None:
            ...
            self.feat_gen_optimizer = eval(self.feat_gen_optim_name)(
                self.global_feat_gen.parameters(), self.feat_gen_optim_lr)
    ...
    def train_global_inference_model(self):
        """Train global inference model using DDPM synthetic data with FedTED MSE+CE loss."""
        ...
        for d_name, mapping in self.local_id_to_global_id.items():
            for l_id, g_id in mapping.items():
                all_global_ids.add(g_id)
        all_global_ids = list(all_global_ids)
        num_global_classes = max(all_global_ids) + 1  # use max+1 for consistent index range
        ...
        # build DDPM generators from aggregated weights
        generators = {}
        ...
        img_size = (
            self.exp_conf.get('channels', 3),
            self.exp_conf.get('img_size', 32),
            self.exp_conf.get('img_size', 32)
        )

        # generate synthetic dataset from DDPM
        dataset_x = []
        dataset_y = []
        ...
        for d_name, mapping in self.local_id_to_global_id.items():
            ...
            for local_id, global_id in mapping.items():
                with torch.no_grad():
                    ...
                    # DDPM is trained on data normalized to [-1, 1], clamp to same range
    ...
    def gen_prox_data(self, num_global_classes):
        """Generate prototype embeddings for each global class via ConditionalGenerator."""
        prox_z = []
    ...
    def evaluate_private(self):
        # use the client-side personalized model
        acc_list, loss_list = [], []
        ...
        for client in self.selected_clients:
            ...
            self.logger.log(f"Round {self.glob_iter + 1} | Client {client.id} | "
                            f"Model: ({client.model_name}) | Acc: {client.round_test_acc*100:.2f}%")
    ...
    def save_model(self, fname='checkpoints.pth'):
        self.logger.log("Saving FedTED+DDPM Server Checkpoints ...")
        ...
        checkpoint = {
            'generator': self.global_feat_gen.state_dict() if self.global_feat_gen is not None else None,
        ...
        # save aggregated DDPM weights
        gen_dir = os.path.join(self.logger.log_dir, 'global_ddpms')
        os.makedirs(gen_dir, exist_ok=True)
        for d_name, state_dict in self.global_ddpm_states.items():
            gan_save_path = os.path.join(gen_dir, f'{d_name}_DDPM.pth')
            torch.save({'generator': state_dict}, gan_save_path)
        ...
        for client in self.clients:
            ...
            client_path = os.path.join(clients_dir,
