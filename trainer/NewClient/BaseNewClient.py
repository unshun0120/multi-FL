...
from utils.test_utils import (
    initialize_new_clients, load_checkpoint, evaluate_client
)
...
class BaseNewClientTrainer:
    def __init__(self, mode_name, args):
        ...
        self.args.num_train_usps = self.saved_args.get('num_train_usps', 0)
    ...
    def _run_single_dataset(self):
        ...
        for client in new_clients:
            ...
            label_to_global_id = self.global_registry[d_name]
            client_curves = {}
            ...
            csv_file_path = os.path.join(self.save_dir, f"{d_name}_newclient_history.csv")
            with open(csv_file_path, mode='w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["Epoch", "Accuracy"])
                for ep, acc in enumerate(client_curves[arch_id]):
                    writer.writerow([ep, acc])
            self.logger.log(f"Accuracy history saved to {csv_file_path}")
    ...
    def _run_super_dataset(self):
        ...
        csv_file_path = os.path.join(self.save_dir, f"SuperDataset_newclient_history.csv")
        with open(csv_file_path, mode='w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Epoch", "Accuracy"])
            for ep, acc in enumerate(client_curves[arch_id]):
...
def get_base_parser():
    ...
    parser.add_argument("--new_client_epochs", type=int, default=10)
    return parser