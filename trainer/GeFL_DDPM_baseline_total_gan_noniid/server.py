...
class Server(BaseServer):
    ...
    def aggregate(self):
        ...
        if (self.glob_iter + 1) % 5 == 0:
            ...
            for entropy_ratio in entropy_ratios:
                ...
                elif self.args.label_mapping == 'slam_dunk':
                ...
                with open(csv_filename, mode='a', newline='', encoding='utf-8') as f:
                    ...
                    writer.writerow([(self.glob_iter + 1), entropy_ratio,
    ...
    def save_model(self, global_round=0, fname='checkpoints.pth'):
        ...
        server_save_path = os.path.join(self.logger.log_dir, f'server_checkpoints_{global_round}.pth')
        torch.save(checkpoint, server_save_path)
        self.logger.log(f"[Server] Checkpoint saved to {server_save_path}")

        gan_dir = os.path.join(self.logger.log_dir, f'local_gans_round_{global_round}')
        ...
        for client in self.clients:
            ...
            gan_save_path = os.path.join(
                gan_dir,
                f'client_GAN_{client.dataset_name}_c{client.id}.pth'
            )

            torch.save(gan_checkpoint, gan_save_path)
            self.logger.log(
                f"[Server] Local GAN for client {client.id} saved to {gan_save_path}"
            )
