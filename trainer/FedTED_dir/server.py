...
class Server(Base_Server):
    ...
    def distribute_model(self):
        ...
        if self.global_feat_gen is not None:
            ...
            if len(all_global_ids) > 0:
                ...
                prox_z = prox_z_tensor.to(self.device).clone().cpu()
    ...
    def train_generator(self):
        ...
        for epoch in tqdm(range(self.global_feat_gen_epochs), colour="blue"):
            ...
            batch_loss_expert = 0.0
            batch_loss_observer = 0.0
            count_expert = 0
            count_observer = 0
            ...
            for client in self.selected_clients:
                ...
                if mask_expert.any():
                    valid_feat = gen_feat[mask_expert]
                    valid_targets = target_tensor[mask_expert]
                    
                    logits = client.model.classifier(valid_feat)
                    
                    loss_ce = criterion(logits, valid_targets)
                    batch_loss_expert += loss_ce
                    count_expert += 1

                if (~mask_expert).any():
                    ood_feat = gen_feat[~mask_expert]
                    ood_logits = client.model.classifier(ood_feat)
            ...
            gen_observer_weight = getattr(self, "gen_observer_weight", 1.0)
            total_cls_loss = batch_loss_expert + (gen_observer_weight * batch_loss_observer)

            total_loss = self.feat_gen_div_beta * div_loss + total_cls_loss
    ...
    def save_model(self, fname='checkpoints.pth'):
        self.logger.log("Saving FedTED_dir Server Checkpoints ...")
